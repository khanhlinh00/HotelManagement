...
namespace Data_Access
{
    public class RoomDAO : BaseDAL
    {
        ...
        public void AddNew(RoomObject room)
        {
            try
            {
        ...
        public void Remove (int roomID)
        {
            try
            {
                RoomObject pro = GetRoomByID(roomID);
                if (pro == null)
                {
                    string SQLInsert = "Delete Room where RoomID = @RoomID";
                    var parameters = new List<SqlParameter>();
                    parameters.Add(DataProvider.CreateParameter("@RoomID", 4, roomID, DbType.Int32));
                    DataProvider.Insert(SQLInsert, CommandType.Text, parameters.ToArray());
                }
                else
                {
